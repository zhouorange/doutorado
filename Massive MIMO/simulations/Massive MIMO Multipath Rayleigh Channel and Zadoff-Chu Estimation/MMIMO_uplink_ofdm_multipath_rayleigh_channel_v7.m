...
for idx = 1:length(snr)
    ...
    while(((nErrs_mrc < nErrors) || (nErrs_zf_le < nErrors) || (nErrs_mmse_le < nErrors) || (nErrs_mfb < nErrors)) && (nBits_mfb < nTotalOfBits))
        ...
        if(ofdm_symbol_number > numSymbInSubframe)
            ...
            H_est = H_estimated(:,:,1);
            G = (H_est*H_est' + (1/linearSnr)*eye(M))^-1;
            G = H_est'*G;
            r_mmse_le = G*ofdm_rx;
...
fileName = sprintf('M_MU_MIMO_M%s_K%s_multFad_woLargeScl_estOneTapChan_%s.mat',m_rx_antennas,m_tx_antennas,timeStamp);
save(fileName);

% Save figure to FIG-file.
fileName = sprintf('M_MU_MIMO_M%s_K%s_multFad_woLargeScl_estOneTapChan_%s.fig',m_rx_antennas,m_tx_antennas,timeStamp);
savefig(h,fileName);
