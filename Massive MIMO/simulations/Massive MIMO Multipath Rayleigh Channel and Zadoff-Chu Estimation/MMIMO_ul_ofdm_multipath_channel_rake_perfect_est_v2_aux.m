...
[BER_MRC, BER_ZF_LE, BER_MMSE_LE] = deal(zeros(1, length(EbNoVec)));
...
for idx = 1:length(snr)
    ...
    while(((nErrs_mrc < nErrors) || (nErrs_zf_le < nErrors) || (nErrs_mmse_le < nErrors)) && (nBits < nTotalOfBits))
    ...
    legend('MRC', 'ZF-LE', 'MMSE-LE');
    drawnow;
    
end

% Draw the lines.
semilogy(EbNoVec, BER_MRC, 'k-', EbNoVec, BER_ZF_LE, 'b-', EbNoVec, BER_MMSE_LE, 'r-');
hold off;
