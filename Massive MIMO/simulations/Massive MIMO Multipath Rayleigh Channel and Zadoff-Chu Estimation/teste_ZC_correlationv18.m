...
M = 1500;                                                              % Number of antennas at the base station of each cell (In this case, i.e., uplink, it gives the number of receiving antennas).
...
if(1) % Apenas um pico por vez
    ...
    for mm=1:1:M
        for kk=1:1:length(position)         
            pdp_freq_adjusted(mm,position(kk)) = PDP_ADJUST_FACTOR(1)*pdp_freq_adjusted(mm,position(kk));
            if(numPaths > 0)
                pdp_freq_adjusted(mm,position(kk)+delay_zcorr1) = (PDP_ADJUST_FACTOR(2))*pdp_freq_adjusted(mm,position(kk)+delay_zcorr1);
            end
        end
    end
