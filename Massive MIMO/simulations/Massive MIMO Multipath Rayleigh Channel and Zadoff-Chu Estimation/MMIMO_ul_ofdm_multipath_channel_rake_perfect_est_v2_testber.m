...
for idx = 1:length(snr)
    ...
    idx_ch = 1;
    iter = 1;
    x = complex(zeros(M,NFFT+NCP+(pos(length(pos))-1)),zeros(M,NFFT+NCP+(pos(length(pos))-1)));
    aux = complex(zeros(M,NFFT+NCP+(pos(length(pos))-1)),zeros(M,NFFT+NCP+(pos(length(pos))-1)));
    ...
    while(((nErrs_mrc < nErrors) || (nErrs_zf_le < nErrors) || (nErrs_mmse_le < nErrors) || (nErrs_mfb < nErrors)) && (nBits_mfb < nTotalOfBits))
        ...
        for ll=1:1:K
            for lll=1:1:NFFT
                Tx(ll,lll) = complex(lll,lll);
            end
        end
        %***************** DEBUG *****************
        
        % Create OFDM symbol.
        sequence = ifft(Tx,NFFT,2);
        
        res = ifft(complex(1:2048,1:2048));
        
        error = 0;
        for jj=1:1:K
            error = error + sum(res-sequence(jj,:));
        end
        
        sequence = sqrt(NFFT)*sequence;
        
        %***************** DEBUG *****************    
        sequence = complex(zeros(K,NFFT),zeros(K,NFFT));
        for ll=1:1:K
            for lll=1:1:NFFT
                sequence(ll,lll) = complex(lll,lll);
            end
        end        
        %***************** DEBUG *****************
...
%         %***************** DEBUG *****************
%         ofdm = complex(zeros(K,1),zeros(K,1));
%         for ll=1:1:K
%             ofdm(ll,1) = complex(ll,ll);  
%         end
%         %***************** DEBUG *****************

        %***************** DEBUG *****************
        ofdm = complex(zeros(K,NFFT+NCP),zeros(K,NFFT+NCP));
        for ll=1:1:K
            ofdm(ll,1) = 1;  
        end
        %***************** DEBUG *****************
        ...
        r_first_finger_path = [r, complex(zeros(M,(pos(length(pos))-1)),zeros(M,(pos(length(pos))-1)))];
                
        % get the second path signal.
        %r_second_finger_path = r(:,pos(length(pos)):end);
        r_second_finger_path = [complex(zeros(M,(pos(k)-1)),zeros(M,(pos(k)-1))), r];
...
%         ofdm_rx_finger2 = (1/sqrt(NFFT))*fft(rx_finger2,NFFT,2);
        ...
        G1 = H(:,:,1)^(-1); %((H(:,:,1)'*H(:,:,1))^(-1))*H(:,:,1)';
        r_eq1 = G1*r_first_finger_path;
        
%         res = sequence;%sqrt(NFFT)*res;
%         error1 = 0;
%         for jj=1:1:K
%             error1 = error1 + sum(res(jj,:)-r_eq1(jj,:));
%         end
        
        
        % Apply ZF-LE to second path received signal.
        G2 = H(:,:,2)^(-1); %((H(:,:,2)'*H(:,:,2))^(-1))*H(:,:,2)';
        r_eq2 = G2*r_second_finger_path;
        
%         res = sequence;%sqrt(NFFT)*res;
%         error2 = 0;
%         for jj=1:1:K
%             error2 = error2 + sum(res(jj,:)-r_eq2(jj,:));
%         end        
        ...
        % Retrieve modulation symbols from first finger path.
        ofdm_combined_rx = (1/sqrt(NFFT))*fft(r_eq_sum_zf_le,NFFT,2);
        ...
        for jj=1:1:K
            demoded_zf_le = demodulate(hDemod, ofdm_combined_rx(jj,:));
