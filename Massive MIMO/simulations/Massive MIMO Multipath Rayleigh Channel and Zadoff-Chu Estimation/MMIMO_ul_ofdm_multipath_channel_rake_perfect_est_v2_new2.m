...
for idx = 1:length(snr)
    ...
    while(((nErrs_mrc < nErrors) || (nErrs_zf_le < nErrors) || (nErrs_mmse_le < nErrors) || (nErrs_mfb < nErrors)) && (nBits_mfb < nTotalOfBits))
        ...
        ofdm_rx_mrc = (1/sqrt(NFFT))*fft(r_eq_sum_mrc,NFFT,2);          
        
        % Iterate over all Tx antennas.
        for jj=1:1:K
            demoded_mrc = demodulate(hDemod, ofdm_rx_mrc(jj,:));
        ...
        % Retrieve modulation symbols from first finger path.
        ofdm_rx_zf = (1/sqrt(NFFT))*fft(r_eq_sum_zf_le,NFFT,2);        
        
        % Iterate over all Tx antennas.
        for jj=1:1:K
            demoded_zf_le = demodulate(hDemod, ofdm_rx_zf(jj,:));
        ...
        % Retrieve modulation symbols from first finger path.
        ofdm_rx_mmse_le = (1/sqrt(NFFT))*fft(r_eq_sum_mmse_le,NFFT,2);         
        
        % Iterate over all Tx antennas.
        for jj=1:1:K
            demoded_mmse_le = demodulate(hDemod, ofdm_rx_mmse_le(jj,:));
        ...
        % Apply MRC to first finger path received signal.
        G1 = H(:,1,1)';
        r_eq1_mfb = G1*rx_finger1_mfb;
        
        % Apply MRC to second finger path received signal.
        G2 = H(:,1,2)';
        r_eq2_mfb = G2*rx_finger2_mfb;
        
        % Combine all the path signals by averaging them.
        r_eq_sum_mfb = (r_eq1_mfb + r_eq2_mfb)/2;   
        
        % Retrieve modulation symbols from first finger path.
        ofdm_rx_mfb = (1/sqrt(NFFT))*fft(r_eq_sum_mfb,NFFT,2);       
        
        % Demodulate signal.
        E_mfb = demodulate(hDemod, ofdm_rx_mfb);
        % *****************************************************************************
