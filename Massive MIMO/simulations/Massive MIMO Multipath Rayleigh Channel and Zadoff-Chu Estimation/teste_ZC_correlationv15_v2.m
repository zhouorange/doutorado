...
% if(1)
%     y_channel = complex(zeros(1,NSUBFRAME),zeros(1,NSUBFRAME));
%     for kk=1:1:K
%         y_channel = y_channel + preambles(kk,:);
%     end
%     
...
if(0)
    for kk=1:1:K
        aux = zu(position(kk)) - complex(1,0);
        zu(position(kk)) = zu(position(kk)) - aux;
        if(delay_zcorr1 > 0)
            aux = zu(position(kk)+delay_zcorr1) - complex(1,0);
            zu(position(kk)+delay_zcorr1) = zu(position(kk)+delay_zcorr1) - aux;            
        end
    end
end
