...
if(1) % Apenas um pico por vez
    ...
    for kk=1:1:length(position)
        
        pdp_freq_adjusted(position(kk)) = PDP_ADJUST_FACTOR(1)*pdp_freq_adjusted(position(kk));
        if(numPaths > 0)
            pdp_freq_adjusted(position(kk)+delay_zcorr1) = (PDP_ADJUST_FACTOR(2))*pdp_freq_adjusted(position(kk)+delay_zcorr1);
        end     
    end
    ...
    for jj=1:1:length(delay)
        for mm=1:1:M
            for kk=1:1:K
                fprintf(1,'h(%d,%d,%d): %s - pdp_freq_adjusted(%d): %s - error: %f\n',mm,kk,jj,num2str(H(1,kk,jj)),(position(kk)+((pos(jj)-1)/30)),num2str(pdp_freq_adjusted(position(kk)+((pos(jj)-1)/30))),abs(H(1,kk,jj)-pdp_freq_adjusted(position(kk)+((pos(jj)-1)/30))));
            end
        end
    end
end
