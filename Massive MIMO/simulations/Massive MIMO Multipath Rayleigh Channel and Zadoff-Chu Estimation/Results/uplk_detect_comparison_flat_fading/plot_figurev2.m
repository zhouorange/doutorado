...
for k_idx=1:1:length(K)
    ...
    for m_idx=1:1:length(M)
        ...
        for idx = 1:length(snr)
            ...
            legend('EGC', 'MRC', 'ZF-LE', 'MMSE-LE', 'ZF-DF', 'MMSE-DF', 'ZF-SIC', 'MMSE-SIC', 'MFB');
            drawnow;
        end
        
        semilogy(EbNoVec(1:idx), BER_EGC(m_idx,1:idx), 'b-', EbNoVec(1:idx), BER_MRC(m_idx,1:idx), 'r-', EbNoVec(1:idx), BER_ZF_LE(m_idx,1:idx), 'b-', EbNoVec(1:idx), BER_MMSE_LE(m_idx,1:idx), 'k-', EbNoVec(1:idx), BER_ZF_DF(m_idx,1:idx), 'r-', EbNoVec(1:idx), BER_MMSE_DF(m_idx,1:idx), 'k-', EbNoVec(1:idx), BER_ZF_SIC(m_idx,1:idx), 'b-', EbNoVec(1:idx), BER_MMSE_SIC(m_idx,1:idx), 'r-', EbNoVec(1:idx), BER_MFB(m_idx,1:idx), 'k-','LineWidth',1,'MarkerFaceColor','white','MarkerSize',8);
