...
M = 10;                                                             % Number of antennas at the base station of each cell (In this case, i.e., uplink, it gives the number of receiving antennas).
...
for idx = 1:length(snr)
    ...
    while(((nErrs_mrc < nErrors) || (nErrs_zf_le < nErrors) || (nErrs_mmse_le < nErrors) || (nErrs_mfb < nErrors)) && (nBits_mfb < nTotalOfBits))
        ...
        if(ofdm_symbol_number > numSymbInSubframe)
            ...
            [ID, TA] = detectPreambleIDAndTAv2(r, M);
...
% Draw the lines
semilogy(EbNoVec, BER_MRC, 'k-', EbNoVec, BER_ZF_LE, 'b-', EbNoVec, BER_MMSE_LE, 'k-', EbNoVec, BER_MFB, 'y-');
hold off;
...
fileName = sprintf('Massive_MU_MIMO_M%s_K%s_multipath_fading_various_detectors_without_large_scale_%s.mat',m_rx_antennas,m_tx_antennas,timeStamp);
save(fileName);

% Save figure to FIG-file.
fileName = sprintf('Massive_MU_MIMO_M%s_K%s_multipath_fading_various_detectors_without_large_scale_%s.fig',m_rx_antennas,m_tx_antennas,timeStamp);
savefig(h,fileName);
