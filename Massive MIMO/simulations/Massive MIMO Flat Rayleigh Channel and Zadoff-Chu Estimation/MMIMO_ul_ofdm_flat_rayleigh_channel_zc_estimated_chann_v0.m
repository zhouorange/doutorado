...
for idx = 1:length(snr)
    ...
    while(((nErrs_mrc < nErrors) || (nErrs_zf_le < nErrors) || (nErrs_mmse_le < nErrors) || (nErrs_mfb < nErrors)) && (nBits_mfb < nTotalOfBits))
        ...
        if(ofdm_symbol_number==1)
            H = (randn(hStr, M, K) +  1i*randn(hStr, M, K))/sqrt(2);
        end
        ...
        if(ofdm_symbol_number > numSymbInSubframe)
            ...
            [ID, TA, H_estimated] = detectPreambleIDAndTAv5(r, M, K);
            
            error = sum(sum(abs(H-H_estimated)))/(M*K);
...
fileName = sprintf('M_MU_MIMO_M%s_K%s_flatfading_zc_est_chann_v0_%s_.fig',m_rx_antennas,m_tx_antennas,timeStamp);
savefig(h,fileName);

% Save workspace to MAT-file.
clear h
fileName = sprintf('M_MU_MIMO_M%s_K%s_flatfading_zc_est_chann_v0_%s.mat',m_rx_antennas,m_tx_antennas,timeStamp);
save(fileName);
