...
function [ID, TA, H_estimated] = detectPreambleIDAndTAv6(rx, M, K, var_channel, var_noise)
...
% ------- Squared modulus used for peak detection (MMSE). -------
NIFFT_CORR = 839;

%mmse_factor = (var_noise*NIDFT)/Nzc;
%mmse_factor = NIDFT*var_noise;
mmse_factor = (Nzc.^2) + ((NIDFT*var_noise)/1);
pdp_freq = ifft(multiplied_sequences,NIFFT_CORR,2) / (mmse_factor);
