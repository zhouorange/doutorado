...
SNR = -22:2:2;                     % Signal-to-noise ratio in dB.
...
for q_idx=1:1:length(q)
    ...
    for beta_iter=1:1:NUM_ITER_BETA
        ...
        for k_idx=1:1:K
            ...
            beta11k_hat1 = beta11k*(1 + sqrt(0.01)*randn(1));
            ...
            % 4) Proposed estimation - sigma^2 = 0.01.
...
hold on;
semilogy(SNR,theoretical_prop_error_vec,'--b*','MarkerSize',7);
semilogy(SNR,real(prop_error_vec_hat1),'--k*','MarkerSize',7);
hold off
grid on;
...
xlabel('SNR [dB]')
ylabel('MSE')
legend('MMSE (ana)','Prop. (ana)','Prop. (sim) \sigma^{2}=0.01');

% % Get timestamp for saving files.
% timeStamp = datestr(now,30);
% fileName = sprintf('channel_estimation_mse_vs_tx_snr_M%d_K%d_P%d_L%d_N%d_v26_%s.fig',M,K,P,L,N,timeStamp);
% savefig(fdee_figure,fileName);
% 
% % Save workspace to .mat file.
% clear fdee_figure
% fileName = sprintf('channel_estimation_mse_vs_tx_snr_M%d_K%d_P%d_L%d_N%d_v26_%s.mat',M,K,P,L,N,timeStamp);
% save(fileName);
