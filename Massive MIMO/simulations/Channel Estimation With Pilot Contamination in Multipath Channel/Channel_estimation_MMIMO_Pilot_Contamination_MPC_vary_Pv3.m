...
for p_idx=1:1:length(P)
    ...
    for m_idx=1:1:length(M)
        ...
        semilogy(P(1:p_idx),real(theoretical_mmse_error(1:p_idx)),'r*', ...
            P(1:p_idx),real(theoretical_prop_error(1:p_idx)),'k*' ,'MarkerSize',7)
        legend('MMSE (ideal)', 'Prop. (ana.)');
        drawnow;
...
semilogy(P,real(theoretical_mmse_error),'r-', ...
    P,real(theoretical_prop_error),'k-' ,'MarkerSize',7)
hold off;

% Get timestamp for saving files.
timeStamp = datestr(now,30);
fileName = sprintf('channel_estimation_mse_vs_P_K%d_M%d_L%d_N%d_v3_%s.fig',K,M,L,N,timeStamp);
savefig(fdee_figure,fileName);

% Save workspace to .mat file.
clear fdee_figure
fileName = sprintf('channel_estimation_mse_vs_P_K%d_M%d_L%d_N%d_v3_%s.mat',K,M,L,N,timeStamp);
save(fileName);
