...
P = 1:2:31;            % Channel Length (Finite Impulse Response - FIR).
L = 7;             % Number of cells.
...
prop_1_error_vec = zeros(1,length(P));
prop_2_error_vec = zeros(1,length(P));
prop_3_error_vec = zeros(1,length(P));
prop_4_error_vec = zeros(1,length(P));
...
for p_idx=1:1:length(P)
    ...
    for m_idx=1:1:length(M)
        ...
        for q_idx=1:1:length(q)
            ...
            prop_error1 = complex(zeros(P(p_idx),P(p_idx)),zeros(P(p_idx),P(p_idx)));
            prop_error2 = complex(zeros(P(p_idx),P(p_idx)),zeros(P(p_idx),P(p_idx)));
            prop_error3 = complex(zeros(P(p_idx),P(p_idx)),zeros(P(p_idx),P(p_idx)));
            prop_error4 = complex(zeros(P(p_idx),P(p_idx)),zeros(P(p_idx),P(p_idx)));
            ...
            ls_error_vec(p_idx) = epsilon11 - beta111;
            ...
            prop_1_error_vec(p_idx) = sum(diag(prop_error1))/P(p_idx);
            
        end
        
        % Plot results. 
        semilogy(P(1:p_idx),real(mmse_error_vec(1:p_idx)),'r*', ...
            P(1:p_idx),real(ls_error_vec(1:p_idx)),'b*', ...
            P(1:p_idx),real(prop_1_error_vec(1:p_idx)),'k*' ,'MarkerSize',7)
        legend('MMSE (ideal)', 'LS (ideal)', 'Prop. (sim)');
        drawnow;
...
semilogy(P,real(mmse_error_vec),'r-', ...
    P,real(ls_error_vec),'b-', ...
    P,real(prop_1_error_vec),'k-' ,'MarkerSize',7)
hold off;
