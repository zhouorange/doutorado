...
theoretical_mmse_error = zeros(1,length(P));
theoretical_ls_error = zeros(1,length(P));
theoretical_prop_error = zeros(1,length(P));
...
for p_idx=1:1:length(P)
    ...
    for m_idx=1:1:length(M)
        ...
        for q_idx=1:1:length(q)
            ...
            % Theoretical Least Squares Error (LS).
            theoretical_ls_error(p_idx) = (epsilon11 - beta111);
            
            % Theoretical Minimum Mean Squared Error (MMSE) error.
            theoretical_mmse_error(p_idx) = (beta111/epsilon11) * (epsilon11 - beta111);
            
            % Theoretical Proposed Estimator Error (Prop.).
            theoretical_prop_error(p_idx) = beta111*( ( ((2-M(m_idx)*P(p_idx))*beta111)/((P(p_idx)*M(m_idx)-1)*epsilon11)  ) + 1);
            
        end
        
        % Plot results. 
        semilogy(P(1:p_idx),real(theoretical_mmse_error(1:p_idx)),'r*', ... 
            P(1:p_idx),real(theoretical_ls_error(1:p_idx)),'k*' , ... 
            P(1:p_idx),real(theoretical_prop_error(1:p_idx)),'bo', 'MarkerSize',7)
...
semilogy(P,real(theoretical_mmse_error),'r-', ...
    P,real(theoretical_ls_error),'k-' , ... 
    P,real(theoretical_prop_error),'b-' ,'MarkerSize',7)
hold off;

% Get timestamp for saving files.
timeStamp = datestr(now,30);
fileName = sprintf('channel_estimation_mse_vs_P_K%d_M%d_L%d_N%d_v4_%s.fig',K,M,L,N,timeStamp);
savefig(fdee_figure,fileName);

% Save workspace to .mat file.
clear fdee_figure
fileName = sprintf('channel_estimation_mse_vs_P_K%d_M%d_L%d_N%d_v4_%s.mat',K,M,L,N,timeStamp);
save(fileName);
