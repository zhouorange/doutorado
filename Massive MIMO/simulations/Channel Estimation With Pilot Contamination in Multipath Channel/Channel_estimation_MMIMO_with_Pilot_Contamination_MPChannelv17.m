...
SNR = -20:2:22;                           % Signal-to-noise ratio in dB.
...
for a_idx=1:1:length(a)
    ...
    %[distances, angles, beta, beta111, beta_sum] = GenerateRandomBetav2(L,K,cellRadius,cellHole,sshadow,gamma);
...
fileName = sprintf('channel_estimation_mse_vs_tx_snr_M%d_K%d_P%d_L%d_N%d_v17_%s.fig',M,K,P,L,N,timeStamp);
savefig(fdee_figure,fileName);

% Save workspace to .mat file.
clear fdee_figure
fileName = sprintf('channel_estimation_mse_vs_tx_snr_M%d_K%d_P%d_L%d_N%d_v17_%s.mat',M,K,P,L,N,timeStamp);
save(fileName);
