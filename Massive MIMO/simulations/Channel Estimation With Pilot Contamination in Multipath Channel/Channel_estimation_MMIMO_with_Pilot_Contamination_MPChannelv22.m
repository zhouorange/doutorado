...
for q_idx=1:1:length(q)
    ...
    ls_error = ls_error./(M * NUM_ITER_BETA);
    ...
    mmse_error = mmse_error./(M * NUM_ITER_BETA);
    ...
    prop_error = prop_error./(M * NUM_ITER_BETA);
...
fileName = sprintf('channel_estimation_mse_vs_tx_snr_M%d_K%d_P%d_L%d_N%d_v22_%s.fig',M,K,P,L,N,timeStamp);
savefig(fdee_figure,fileName);

% Save workspace to .mat file.
clear fdee_figure
fileName = sprintf('channel_estimation_mse_vs_tx_snr_M%d_K%d_P%d_L%d_N%d_v22_%s.mat',M,K,P,L,N,timeStamp);
save(fileName);
