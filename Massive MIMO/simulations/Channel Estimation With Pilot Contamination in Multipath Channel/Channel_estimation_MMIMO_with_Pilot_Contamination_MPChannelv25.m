...
prop_error_vec_hat3 = zeros(1,length(q));
for q_idx=1:1:length(q)
    ...
    prop_error_hat3 = complex(zeros(P,P),zeros(P,P));
    for beta_iter=1:1:NUM_ITER_BETA
        ...
        for k_idx=1:1:K
            ...
            beta11k_hat3 = beta11k*(1 + sqrt(0.001)*randn(1));
            ...
            % 6) Proposed estimation - sigma^2 = 0.001.
            Z11_prop_hat3 = (M*beta11k_hat3*Z11_ls)./trace_aux_by_p;
            
            prop_error_hat3 = prop_error_hat3 + ((Z11_prop_hat3'-G11k')*(Z11_prop_hat3-G11k));
    ...
    % Proposed Estimator Simulation Error - sigma^2 = 0.001. (Monte Carlo)
    prop_error_hat3 = prop_error_hat3./(M * NUM_ITER_BETA * K);
    prop_error_vec_hat3(q_idx) = sum(diag(prop_error_hat3))/P;
...
semilogy(SNR,real(prop_error_vec_hat3),'k*','MarkerSize',7);
hold off
grid on;
...
xlabel('SNR [dB]')
ylabel('MSE')
legend('MMSE (ana)','LS (ana)','Prop. (ana)','Prop. (sim)','Prop. (sim) \sigma^{2}=0.1','Prop. (sim) \sigma^{2}=0.01','Prop. (sim) \sigma^{2}=0.001');

% Get timestamp for saving files.
timeStamp = datestr(now,30);
fileName = sprintf('channel_estimation_mse_vs_tx_snr_M%d_K%d_P%d_L%d_N%d_v25_%s.fig',M,K,P,L,N,timeStamp);
savefig(fdee_figure,fileName);

% Save workspace to .mat file.
clear fdee_figure
fileName = sprintf('channel_estimation_mse_vs_tx_snr_M%d_K%d_P%d_L%d_N%d_v25_%s.mat',M,K,P,L,N,timeStamp);
save(fileName);
