...
% ofdm parameters
    ...
    ofdm.GI =  16;                      % length of guard interval, also known as Cyclic Prefix (CP).
    ...
    ofdm.NP =  length(ofdm.PP);         % number subcarriers which carry pilots.
...
% channel parameters
    ...
    [~,chan.Delay]  = sort([0,rand(1,chan.Nt)]);    % generating random delay for each tap
...
% building dictionary (please check different papers to learn how to build the dictionary)
    ...
    a = ((1:ofdm.N).');
    idx1 = repmat(a,1,chan.Nt);
    idx2 = repmat(chan.tau_p,ofdm.N,1);
    idx3 = idx1./ofdm.TS.*idx2;
    chan.Gamma = exp(-sqrt(-1)*2*pi.*idx3);
...
for cnt1 = 1 :  loop.End1
    for cnt2 = 1 : loop.End2
        ...
        ofdm.Pilot = P.';  %ones(ofdm.NP,1);% or randsrc(ofdm.NP,ofdm.B,[-1 1]).*exp(-sqrt(-1)*pi*rand(ofdm.NP,ofdm.B));
        ...
        rayleigh_tau = chan.tau_p(chan.Delay(1:chan.L));
        rayleigh_pdp = chan.Gain(chan.Delay(1:chan.L));
        ch = rayleighchan(ofdm.T,chan.fd,rayleigh_tau,rayleigh_pdp);
