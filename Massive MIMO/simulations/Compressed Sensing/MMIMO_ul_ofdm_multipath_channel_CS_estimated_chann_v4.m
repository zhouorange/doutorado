...
for idx = 1:length(snr)
    ...
    while(est_error > 1e-10)
        ...
        sigPower1 = sum(abs(sequence(1,:)).^2)/length(sequence(1,:));
                
        % Add CP.
        ofdm = [sequence(:,NFFT-NCP+1:end), sequence];
                        
        sigPower2 = sum(abs(ofdm(3,:)).^2)/length(ofdm(3,:));
        ...
        for m_idx=1:1:M
            ...
            for k_idx=1:1:K
                ...
                e_ls = sum(abs(h - g_hat_ls).^2)/pos(length(pos));
                
                e_omp = sum(abs(h - g_hat_omp(1:pos(length(pos)))).^2)/pos(length(pos));
                
                e_mmse = sum(abs(h - g_hat_mmse).^2)/pos(length(pos));
                
                % Estimate of H.
                H_hat(m_idx,k_idx,:) = g_hat_ls(pos);
            end
        end
        
        est_error = sum(sum(sum(abs(H_hat - H))))/(M*K*numPaths);
        
        if(est_error < min_error)
            min_error = est_error;
            ppos_min = ppos;
        end
