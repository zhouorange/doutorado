...
% % ************ Normalize channel vectors ************
% maxDelaySpread = numPaths; % pos(length(pos))-1;  
% for idx_ch = 1 : N_chann
%     
%     % Atualizacao das matrizes de canal para a celula alvo (l = 1): existe uma matriz de canal por percurso do terminal.
%     G = reshape(ch(idx_ch,:), M, K, length(pos));
...
%     for m_idx=1:1:M      
%         for k_idx=1:1:K 
%             h = [G(m_idx,k_idx,1), G(m_idx,k_idx,2)];
%             energy = sum(abs(h).^2)/maxDelaySpread;
%             normalization_factor = 1/sqrt(energy);           
%             h = normalization_factor*h;
%             channel_energy = sum(abs(h).^2)/maxDelaySpread;
%             if(channel_energy>1.00001)
%                 error('Normalized Channel energy too large!!!!!');
%             end
%             G(m_idx,k_idx,:) = h;          
%         end
%     end
    ...
    for idx_ch=1:1:N_chann
        ...
        for m_idx=1:1:M
            ...
            for k_idx=1:1:K
                ...
                % MMSE
                invMatMmse = (A'*A + (1/linearSnr)*eye(delaySpreadMax))^-1;
                ...
                avg_error_omp = avg_error_omp + (sum(abs(h - g_hat_omp(1:pos(length(pos)))).^2)/pos(length(pos)));
                
                avg_error_ls = avg_error_ls + (sum(abs(h - g_hat_ls).^2)/pos(length(pos)));
                
                avg_error_mmse = avg_error_mmse + (sum(abs(h - g_hat_mmse).^2)/pos(length(pos)));
            end
        end
        
        est_error_omp  = sum(sum(sum(abs(H_hat_omp - H).^2)))/(M*K*numPaths);
        est_error_ls   = sum(sum(sum(abs(H_hat_ls - H).^2)))/(M*K*numPaths);
        est_error_mmse = sum(sum(sum(abs(H_hat_mmse - H).^2)))/(M*K*numPaths);
        
        avg_error_omp = avg_error_omp/(M*K);
        avg_error_ls = avg_error_ls/(M*K);
        avg_error_mmse = avg_error_mmse/(M*K);
        
        fprintf(1,'Estimation error OMP: %d - avg error: %d\n',est_error_omp, avg_error_omp);
        fprintf(1,'Estimation error LS: %d - avg error: %d\n',est_error_ls, avg_error_ls);
        fprintf(1,'Estimation error MMSE: %d - avg error: %d\n',est_error_mmse, avg_error_mmse);
