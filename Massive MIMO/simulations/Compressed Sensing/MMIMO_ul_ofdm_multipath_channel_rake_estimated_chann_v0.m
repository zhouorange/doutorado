...
for idx = 1:length(snr)
    ...
    while(((nErrs_mrc < nErrors) || (nErrs_zf_le < nErrors) || (nErrs_mmse_le < nErrors) || (nErrs_mfb < nErrors)) && (nBits_mfb < nTotalOfBits))
        ...
        x = H(:,:,1)*signal;
        for k = 2:length(pos)
            aux = [complex(zeros(M,(pos(k)-1)),zeros(M,(pos(k)-1))) H(:,:,k)*signal(:,1:end-(pos(k)-1))];
            x = x + aux;
        end
        ...
        if(ofdm_symbol_number > numSymbInSubframe)
            ...
            H_est1 = H_estimated(:,:,1);
            G1 = ((H_est1'*H_est1)^(-1))*H_est1';
            r_eq1 = G1*ofdm_rx_finger1;
            
            % Apply ZF-LE to second path received signal.
            H_est2 = H_estimated(:,:,2);
            G2 = ((H_est2'*H_est2)^(-1))*H_est2';
            r_eq2 = G2*ofdm_rx_finger2;
            ...
            [ID, TA, H_estimated] = detectPreambleIDAndTAv4(r, M, K, numPaths, pos);         
...
fileName = sprintf('M_MU_MIMO_M%s_K%s_multFad_woLargeScl_rake_est_chann_%s.fig',m_rx_antennas,m_tx_antennas,timeStamp);
savefig(h,fileName);

% Save workspace to MAT-file.
clear h
fileName = sprintf('M_MU_MIMO_M%s_K%s_multFad_woLargeScl_rake_est_chann_%s.mat',m_rx_antennas,m_tx_antennas,timeStamp);
save(fileName);

% Simulation: Massive Multi User MIMO Multipath-fading without Large Scale Fadind and Estimated One Tap Channel
