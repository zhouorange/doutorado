...
numOfUserPilotsPerSymbols = 5;                                                                      % Number of pilot sets per OFDM symbol.
Np = floor(NFFT/numOfUserPilotsPerSymbols);                                                         % Number of pilots per OFDM symbol. (use prime numbers)
...
numSym = K*NFFT;                                                                                    % Number of symbols, i.e., number of terminals. The number of pilots must be leaved out.
...
EbNoVec = 1000; %-10:2:6;%-20:2:40;                                                                    % Eb/No in dB.
...
%% @@@@@@@@@@@@@@@@ Create Pilots @@@@@@@@@@@@@@@@@@@@@@@@@@@@
div = numOfUserPilotsPerSymbols;
pilot_pos = zeros(div,Np);
pilot_pos(1,:) = 1:div:NFFT-div;
for k=2:1:div
    pilot_pos(k,:) = pilot_pos(1,:) + (k-1);
end

P = complex(zeros(div,Np),zeros(div,Np));
Pilots = zeros(div,NFFT);
for jj=1:1:div
    ip = rand(1,Np) > 0.5; % generating 0,1 with equal probability
    P(jj,:) = 2*ip-1; % BPSK modulation 0 -> -1; 1 -> 0
    Pilots(jj,pilot_pos(jj,:)) = P(jj,:);
end

numberOfOFDMSymbolsForPilots = K/numOfUserPilotsPerSymbols;
...
[BER_MMSE_LS, BER_MMSE_OMP, BER_MMSE_OPT, BER_MMSE_ELS, BER_MMSE_GAELS] = deal(zeros(1, length(EbNoVec)));
...
for snr_idx = 1:1:length(snr)
    ...
    while(((nErrs_mmse_ls < nErrors) || (nErrs_mmse_omp < nErrors) || (nErrs_mmse_opt < nErrors) || (nErrs_mmse_els < nErrors) || (nErrs_mmse_gaels < nErrors)) && (nBits < nTotalOfBits))
        ...
        %---------- Transmission (UE) ----------
        if(ofdm_symbol_number==1 || ofdm_symbol_number==2)
            % Create OFDM symbol with pilots.
            pilots = (NFFT/sqrt(NFFT))*ifft(Pilots,NFFT,2);
            
            % Add CP
            ofdm = [pilots(:,end-NCP+1:end), pilots];
        else
            ...
            % Split source symbols among K terminals.
        ...
        lpos = K;
        if(ofdm_symbol_number==1 || ofdm_symbol_number==2)
            ipos = (ofdm_symbol_number-1)*numOfUserPilotsPerSymbols + 1;
            lpos = ofdm_symbol_number*numOfUserPilotsPerSymbols;
        end
        
        x = [H(:,ipos:lpos,1)*ofdm complex(zeros(M,(pos(length(pos))-1)),zeros(M,(pos(length(pos))-1)))];
        for k = 2:length(pos)
            aux = [complex(zeros(M,(pos(k)-1)),zeros(M,(pos(k)-1))) H(:,ipos:lpos,k)*ofdm];
            x = x + aux;
        end
        ...
        if(ofdm_symbol_number==1 || ofdm_symbol_number==2)
            for m_idx=1:1:M
                for k_idx=1:1:numOfUserPilotsPerSymbols
                    
                    Pdiag = diag(P(k_idx,:));
                    
                    %% ********* Fourier Basis *********
                    Fl = F(pilot_pos(k_idx,:).',:);
                    
                    y = ofdm_rx(m_idx,pilot_pos(k_idx,:)).';
                    ...
                    % --------- Estimate of H ---------
                    kidx = k_idx + (ofdm_symbol_number - 1)*numOfUserPilotsPerSymbols;
                    H_hat_omp(m_idx,kidx,:)   = g_hat_omp(pos);
                    H_hat_ls(m_idx,kidx,:)    = g_hat_ls(pos);
                    H_hat_gaels(m_idx,kidx,:) = g_hat_gaels(pos);
                    H_hat_els(m_idx,kidx,:)   = g_hat_els(pos);
                end
            end
        else
            ...
            ofdm_rx_finger1 = (sqrt(NFFT)/NFFT)*fft(rx_finger1,NFFT,2);
            
            % Retrieve modulation symbols from second finger path.
            ofdm_rx_finger2 = (sqrt(NFFT)/NFFT)*fft(rx_finger2,NFFT,2);
            ...
            %% ****************************** 6) MMSE GA-ELS receiver *************************
            
            % Apply MMSE-E-LE to fisrt finger path received signal.
            H1_est = H_hat_gaels(:,:,1);
            G1 = (((H1_est'*H1_est) + (1/linearSnr)*eye(K))^(-1));
            G1 = G1*H1_est';
            r_eq1 = G1*ofdm_rx_finger1;
            
            % Apply MMSE-E-LE to second finger path received signal.
            ...
            % Combine all the path signals by averaging them.
            r_eq_sum_mmse_gaels = (r_eq1 + r_eq2)/2;
            
            % Retrieve modulation symbols.
            data_symbols_mmse_gaels = r_eq_sum_mmse_gaels(~flag_data).';
            
            % Demodulate signal.
            E_mmse_gaels = demodulate(hDemod, data_symbols_mmse_gaels);
            % *****************************************************************************
            ...
            % Combine all the path signals by averaging them.
            r_eq_sum_mmse_els = (r_eq1 + r_eq2)/2;
            
            % Retrieve modulation symbols.
            data_symbols_mmse_els = r_eq_sum_mmse_els(~flag_data).';
            
            % Demodulate signal.
            E_mmse_els = demodulate(hDemod, data_symbols_mmse_els);
            % *****************************************************************************
            ...
            nErrs_mmse_els = nErrs_mmse_els + biterr(msg, E_mmse_els);
            nErrs_mmse_gaels   = nErrs_mmse_gaels + biterr(msg, E_mmse_gaels);
            
            nBits = nBits + length(msg(:));
            fprintf(1,'SNR: %d\n',snr(snr_idx));
            fprintf(1,'BER MMSE LS:   %f - nErrs_mmse_ls:   %d - nBits: %d - iter: %d\n',(nErrs_mmse_ls./nBits),nErrs_mmse_ls,nBits,iter);
            fprintf(1,'BER MMSE GA-ELS:   %f - nErrs_mmse_gaels:   %d - nBits: %d - iter: %d\n',(nErrs_mmse_gaels./nBits),nErrs_mmse_gaels,nBits,iter);
            fprintf(1,'BER MMSE ELS: %f - nErrs_mmse_els: %d - nBits: %d - iter: %d\n',(nErrs_mmse_els./nBits),nErrs_mmse_els,nBits,iter);
    ...
    BER_MMSE_ELS(snr_idx) = nErrs_mmse_els./nBits;
    BER_MMSE_GAELS(snr_idx) = nErrs_mmse_gaels./nBits;
    
    % Plot results.
    semilogy(EbNoVec(1:snr_idx), BER_MMSE_LS(1:snr_idx), 'ks', ...
        EbNoVec(1:snr_idx), BER_MMSE_GAELS(1:snr_idx), 'k*', ...
        EbNoVec(1:snr_idx), BER_MMSE_ELS(1:snr_idx), 'ko', ...
        EbNoVec(1:snr_idx), BER_MMSE_OMP(1:snr_idx), 'ro', ...
        EbNoVec(1:snr_idx), BER_MMSE_OPT(1:snr_idx), 'g');
    legend('MMSE LS', 'MMSE GA-ELS', 'MMSE ELS', 'MMSE OMP', 'MMSE OPT');
    drawnow;
    
end

% Draw the lines.
semilogy(EbNoVec, BER_MMSE_LS, 'k-', EbNoVec, BER_MMSE_GAELS, 'k-', EbNoVec, BER_MMSE_ELS, 'k-', EbNoVec, BER_MMSE_OMP, 'r-', EbNoVec, BER_MMSE_OPT, 'g-');
hold off;
...
if(add_awgn)
    ...
    fileName = sprintf('MU_MMIMO_M%s_K%s_Np%s_BER_comparison_%s_mp_ls_v0.fig',m_rx_antennas,m_tx_antennas,n_pilots,timeStamp);
    savefig(figura,fileName); clear figura
    
    % Save workspace to MAT-file.
    fileName = sprintf('MU_MMIMO_M%s_K%s_Np%s_BER_comparison_%s_mp_ls_v0.mat',m_rx_antennas,m_tx_antennas,n_pilots,timeStamp);
    save(fileName);
end