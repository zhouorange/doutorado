...
for q_idx=1:1:length(q)
    ...
    max_data_re = max(max(max(abs(real(Y1)))));
    max_data_im = max(max(max(abs(imag(Y1)))));
    
    max_label_re = max(max(abs(real(g_111))));
    max_label_im = max(max(abs(imag(g_111))));
    ...
    for trainIter = 1:1:numTrainVectors
        ...
        for y_col_idx=1:1:size(Y1,2)
            for y_line_idx=1:1:size(Y1,1)
                idx = idx + 1;
                train_data(trainIter,idx) = real(Y1(y_line_idx,y_col_idx,trainIter))/max_data_re;
                idx = idx + 1;
                train_data(trainIter,idx) = imag(Y1(y_line_idx,y_col_idx,trainIter))/max_data_im;
            end
        end
        ...
        for g_line_idx=1:1:size(g_111,1)
            idx = idx + 1;
            train_label(trainIter,idx) = real(g_111(g_line_idx,trainIter))/max_label_re;
            idx = idx + 1;
            train_label(trainIter,idx) = imag(g_111(g_line_idx,trainIter))/max_label_im;
        end
    end
    ...
    for testIter = 1:1:numTestVectors
        ...
        for y_col_idx=1:1:size(Y1,2)
            for y_line_idx=1:1:size(Y1,1)
                idx = idx + 1;
                test_data(testIter,idx) = real(Y1(y_line_idx,y_col_idx,testIter))/max_data_re;
                idx = idx + 1;
                test_data(testIter,idx) = imag(Y1(y_line_idx,y_col_idx,testIter))/max_data_im;
            end
        end
        ...
        for g_line_idx=1:1:size(g_111,1)
            idx = idx + 1;
            test_label(testIter,idx) = real(g_111(g_line_idx,testIter))/max_label_re;
            idx = idx + 1;
            test_label(testIter,idx) = imag(g_111(g_line_idx,testIter))/max_label_im;
        end
    end
    ...
    for predictionIter = 1:1:numPredictionVectors
        ...
        for y_col_idx=1:1:size(Y1,2)
            for y_line_idx=1:1:size(Y1,1)
                idx = idx + 1;
                prediction_data(predictionIter,idx) = real(Y1(y_line_idx,y_col_idx,predictionIter))/max_data_re;
                idx = idx + 1;
                prediction_data(predictionIter,idx) = imag(Y1(y_line_idx,y_col_idx,predictionIter))/max_data_im;
            end
        end
        ...
        for g_line_idx=1:1:size(g_111,1)
            idx = idx + 1;
            prediction_label(predictionIter,idx) = real(g_111(g_line_idx,predictionIter))/max_label_re;
            idx = idx + 1;
            prediction_label(predictionIter,idx) = imag(g_111(g_line_idx,predictionIter))/max_label_im;
        end
    end
