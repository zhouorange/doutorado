...
for q_idx=1:1:length(q)
    ...
    theoretical_prop_error(q_idx) = (beta111*((((2-M)*beta111)/((M-1)*epsilon11)) + 1));
end
...
semilogy(SNR,real(theoretical_prop_error),'k*','MarkerSize',7);
hold off
grid on;
...
fileName = sprintf('flat_channel_estimation_mse_vs_tx_snr_M%d_K%d_L%d_N%d_v6_%s.fig',M,K,L,N,timeStamp);
savefig(fdee_figure,fileName);

% Save workspace to .mat file.
clear fdee_figure
fileName = sprintf('flat_channel_estimation_mse_vs_tx_snr_M%d_K%d_L%d_N%d_v6_%s.mat',M,K,L,N,timeStamp);
save(fileName);
